...
const Return = ({ setIsLoading, setPage }) => {
  ...
  const handleSubmit = () => {
    ...
    if (Object.values(temp).every((item) => !item)) {
      ...
      let d = new Date(searchInfo.departureDate);
      let t;
      ...
      d = new Date(searchInfo.returnDate);
      switch (searchInfo.returnTime) {
        case "Morning":
          t = "08:00:00";
          break;
        case "Afternoon":
      ...
      reqObject.preferredReturnDepartureTime = `${d.toISOString().split("T")[0]}T${t}`;
      reqObject.preferredReturnArrivalTime = `${d.toISOString().split("T")[0]}T${t}`;
          ...
          <Grid xs={6}>
            ...
            <LocalizationProvider dateAdapter={DateAdapter}>
              <DatePicker
                ...
                onChange={(newValue) => {
                  setSearchInfo({ ...searchInfo, returnDate: newValue });
                }}
                renderInput={(params) => <MKInput {...params} />}
              />
            </LocalizationProvider>
